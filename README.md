...
A full-stack AI-powered system that enables users to ask natural-language questions about multi-speaker meeting transcripts and receive answers **grounded in evidence**, weighted by **speaker reliability scores**.
...
- **Speaker Reliability Scores** — Per-speaker trust computed from behavioural analysis and contradiction detection
...
+-----------+---------------+
            v
+---------------------------+
|  Trust Re-ranking         |  <- similarity x speaker trust score
|  (heuristic + BART-MNLI)  |
+-----------+---------------+
            v
+---------------------------+
...
| [sentence-transformers/all-MiniLM-L6-v2](https://huggingface.co/sentence-transformers/all-MiniLM-L6-v2) | Utterance and query embeddings | ~80 MB |
...
backend/
    ...
    data/dataset_loader.py                 AMI dataset streaming and caching
...
**Note:** The first launch downloads ML models (~2.7 GB) and streams the AMI dataset. This takes 30-60 minutes. Subsequent launches use cached data and start in seconds.
...
| Low Contradiction | 1 minus contradiction ratio (via BART-MNLI NLI) |
